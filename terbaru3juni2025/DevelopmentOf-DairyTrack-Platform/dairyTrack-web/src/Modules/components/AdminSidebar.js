...
const AdminSidebar = ({ collapsed, activeMenu, onMenuToggle }) => {
  ...
  // Function to generate role-based links with safe handling
  const getRoleBasedLink = (path, roleId) => {
    const prefix = getRolePrefix(roleId);
    return path ? `/${prefix}${path}` : "/"; // Default ke "/" jika path undefined
  };
  ...
  const allMenuItems = useMemo(
    ...
    [userRoleId] // Dependency array memastikan ulang hitung saat userRoleId berubah
  );
            ...
            className={`nav-item ${
              ...
              (item.submenu &&
                item.submenu.some(
                  (subItem) =>
                    subItem.link &&
                    location.pathname.toLowerCase() === subItem.link.toLowerCase()
                ))
                ? "active"
                : ""
            }`}
